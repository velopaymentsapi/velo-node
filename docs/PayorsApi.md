...
> PayorLinksResponse payorLinks(descendantsOfPayor, opts)
...
let apiInstance = new VeloPaymentsApIs.PayorsApi();
let descendantsOfPayor = null; // String | The Payor ID from which to start the query to show all descendants
let opts = {
...
apiInstance.payorLinks(descendantsOfPayor, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
